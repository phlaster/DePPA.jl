...
struct GappedOligo <: AbstractGapped
    ...
    function GappedOligo(
        parent_oligo::DegenOligo,
        gaps::Vector{Pair{Int, Int}},
        total_length::Int,
    )
...
struct OligoView{T <: AbstractOligo} <: AbstractOligo
    ...
    function OligoView(oligo::T, interval::UnitRange{Int}) where {T <:
        Union{Oligo, DegenOligo, GappedOligo}}
...
Base.:(==)(a::GappedOligo, b::GappedOligo) = (parent(a) == parent(b)) &&
    (getgaps(a) == getgaps(b)) &&
    (length(a) == length(b))
...
function Base.String(go::GappedOligo)
    ...
    if ungapped_pos <= parent_len || buffer_idx <= L
        throw(ArgumentError(
            "Mismatch in sequence construction: ungapped_pos=$ungapped_pos (expected $(parent_len + 1)), buffer_idx=$buffer_idx (expected $(L + 1))",
        ))
    end
...
Base.codeunit(ov::OligoView, i::Integer) = codeunit(
    parent(ov),
    oligo_range(ov).start + i - 1,
)
...
Base.isvalid(::Type{Oligo}, s::AbstractString) = all(
    c -> uppercase(c) in NON_DEGEN_BASES,
    s,
)
Base.isvalid(::Type{DegenOligo}, s::AbstractString) = all(c -> uppercase(c) in ALL_BASES, s)
Base.isvalid(::Type{GappedOligo}, s::AbstractString) = all(
    c -> uppercase(c) in BASES_W_GAPS,
    s,
)
...
_has_gaps_type(::Type{T}) where {T <: AbstractOligo} = T <:
    Union{GappedOligo, OligoView{<:GappedOligo}}

function Base.promote_rule(::Type{T1}, ::Type{T2}) where {T1 <: AbstractOligo, T2 <:
    AbstractOligo}
...
Base.promote_rule(::Type{<:Union{String, SubString}}, ::Type{T}) where {T <:
    AbstractOligo} = T
Base.promote_rule(::Type{T}, ::Type{<:Union{String, SubString}}) where {T <:
    AbstractOligo} = T
...
n_unique_oligos(ov::OligoView) = reduce(
    *,
    (IUPAC_COUNTS[base] for base in ov),
    init = BigInt(1),
)
...
function sample_max_gc(d::T) where {T <: AbstractOligo}
    ...
    descr = isempty(base_descr) ?
        "Max GC content sample" :
        "Max GC content sample of $base_descr"
...
function sample_min_gc(d::T) where {T <: AbstractOligo}
    ...
    base_descr = description(d)
    descr = isempty(base_descr) ?
        "Min GC content sample" :
        "Min GC content sample of $base_descr"
